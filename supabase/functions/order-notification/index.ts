...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!orderId || !storeId) {
      throw new Error('Missing required fields: orderId and storeId are required');
    }
    ...
    if (!order.store.telefone) {
      throw new Error('Store phone number not found');
    }
    ...
    const orderTrackingUrl = `https://xmenu.pro/checkout/pedidos/${order.id}`;

    // Create personalized message for store owner
    const storeMessage = `🛍️ Novo Pedido Recebido!
...
Olá ${order.customer_name}, seu pedido foi recebido com sucesso!
...
Agradecemos a preferência! 😊`;
    ...
    await Promise.all([
      // Send to store owner
      sendWhatsAppMessage(order.store.telefone, storeMessage),
      // Send to customer
      sendWhatsAppMessage(order.customer_phone, customerMessage)
    ]);
    ...
    // Log error details
    console.error('Error in order-notification function:', {
      error: error instanceof Error ? error.message : 'Unknown error',
    ...
    return new Response(
      JSON.stringify({
        error: error instanceof Error ? error.message : 'Error sending order notification',
