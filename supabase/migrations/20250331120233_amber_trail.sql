/*
  ...
  1. Changes
    - Update manage_subscription_transition function to properly handle subscription status
    - Remove automatic cancellation of existing subscriptions
    - Keep existing subscription active when updating plan
    - Add proper status transition handling
...
-- Create improved function that maintains active status
...
BEGIN
  ...
  BEGIN
    ...
    INSERT INTO subscriptions (
      ...
      p_plan_id,
      p_payment_id,
      'active', -- Always set status to active for new subscriptions
      p_period_type,
      now(),
      CASE p_period_type
    ...
    -- If there was an old subscription, mark it as inactive
