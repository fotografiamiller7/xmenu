/*
  # Add status parameter to subscription transition function

  1. Changes
    - Add p_status parameter to manage_subscription_transition
    - Update function to use provided status
    - Maintain existing validation and constraints
...
-- Create improved function with status parameter
...
BEGIN
  ...
  -- Validate status
  IF p_status NOT IN ('active', 'inactive', 'canceled', 'past_due', 'trialing') THEN
    RAISE EXCEPTION 'Invalid status: %. Must be one of: active, inactive, canceled, past_due, trialing', p_status;
  END IF;
  ...
  -- For paid plans with active status, payment_id is required and must be approved
  IF v_plan_price > 0 AND p_status = 'active' THEN
    IF p_payment_id IS NULL THEN
      RAISE EXCEPTION 'Payment ID is required for paid active plans';
    END IF;
  ...
  BEGIN
    ...
    -- Update profile's plan reference only if status is active
