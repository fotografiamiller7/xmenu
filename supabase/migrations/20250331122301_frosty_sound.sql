/*
  ...
  1. Changes
    - Update manage_subscription_transition function to handle status-only changes
    - Allow status changes without requiring payment ID
    - Maintain payment ID requirement only for new paid subscriptions
...
BEGIN
  ...
  -- Only require payment ID for new paid subscriptions or plan changes
  IF v_plan_price > 0 AND p_status = 'active' AND v_is_plan_change THEN
  ...
  BEGIN
    ...
    IF v_old_subscription IS NOT NULL THEN
      ...
      SET 
        plan_id = p_plan_id,
        payment_id = CASE 
          WHEN v_is_plan_change THEN p_payment_id 
          ELSE s.payment_id 
        END,
        status = p_status,
        period_type = p_period_type,
        current_period_start = CASE 
          WHEN v_is_plan_change THEN now() 
          ELSE s.current_period_start 
        END,
        current_period_end = CASE 
          WHEN v_is_plan_change THEN
            CASE p_period_type
