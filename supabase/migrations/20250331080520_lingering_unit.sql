...
-- Create improved function that properly handles free plans
...
BEGIN
  ...
  -- For paid plans, payment_id is required
  IF v_plan_price > 0 AND p_payment_id IS NULL THEN
    RAISE EXCEPTION 'Payment ID is required for paid plans';
  END IF;

  -- For paid plans, verify payment status
